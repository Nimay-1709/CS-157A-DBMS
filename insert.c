  ...
  if( pColumn==0 && nColumn>0 ){
    ...
    */
    ...
    if( nColumn!=(pTab->nCol-nHidden) ){
        //Following line prints the table name in which insert was tried
        printf("Metadata help: %s{",pTab->zName);
        //Following for loop will be used to print the column names in the table
        for(int i = 0; i < pTab->nCol-nHidden;i++)
        {
            printf("%s",pTab->aCol[i].zName);
            //if-else puts comma between column names and close bracket at the end
            if(i != (pTab->nCol-nHidden-1))
            {
                printf(",");
            }
            else
